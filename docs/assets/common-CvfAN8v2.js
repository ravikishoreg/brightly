import"./style-B7_g1lDc.js";import{q as A}from"./quiz-storage-BNYu41eQ.js";function $(d){let t=d.correctAnswer;return Array.isArray(t)&&(t=t.join(",")),t=String(t),`${d.question.toLowerCase().trim()}_${t.toLowerCase().trim()}`.replace(/[^a-z0-9_]/g,"")}function D(d,t){return d.filter(e=>!t.has($(e)))}const C="your-github-client-id",x=window.location.origin+"/auth-callback.html",k="repo",Q={questionCount:[{value:"2",label:"2 Questions"},{value:"5",label:"5 Questions",selected:!0},{value:"10",label:"10 Questions"},{value:"15",label:"15 Questions"},{value:"20",label:"20 Questions"}],timeLimit:[{value:"0",label:"No Limit",selected:!0},{value:"1",label:"1 Minute"},{value:"2",label:"2 Minutes"},{value:"3",label:"3 Minutes"},{value:"4",label:"4 Minutes"},{value:"5",label:"5 Minutes"},{value:"10",label:"10 Minutes"},{value:"15",label:"15 Minutes"},{value:"20",label:"20 Minutes"}]};function H(){return`
  ...
  `}let g=null,y=null;class S{constructor(t,e,n){this.duration=t,this.onTick=e,this.onComplete=n,this.timeLeft=t,this.interval=null,this.isRunning=!1}start(){this.isRunning=!0,this.interval=setInterval(()=>{this.timeLeft--,this.onTick&&this.onTick(this.timeLeft),this.timeLeft<=0&&(this.stop(),this.onComplete&&this.onComplete())},1e3)}stop(){this.isRunning=!1,this.interval&&(clearInterval(this.interval),this.interval=null)}getTimeLeft(){return this.timeLeft}formatTime(t){const e=Math.floor(t/60),n=t%60;return`${e.toString().padStart(2,"0")}:${n.toString().padStart(2,"0")}`}}class T{static async login(){const t=`https://github.com/login/oauth/authorize?client_id=${C}&redirect_uri=${encodeURIComponent(x)}&scope=${k}`;window.location.href=t}static async logout(){g=null,y=null,localStorage.removeItem("github_token"),localStorage.removeItem("github_user"),this.updateUI()}static async handleCallback(t){try{const e=await fetch("/api/github/auth",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({code:t})});if(e.ok){const n=await e.json();y=n.access_token,g=n.user,localStorage.setItem("github_token",y),localStorage.setItem("github_user",JSON.stringify(g)),this.updateUI()}}catch(e){console.error("Auth error:",e)}}static async loadUser(){const t=localStorage.getItem("github_token"),e=localStorage.getItem("github_user");t&&e&&(y=t,g=JSON.parse(e),this.updateUI())}static updateUI(){const t=document.getElementById("auth-section");t&&(g?t.innerHTML=`
        <span>Welcome, ${g.login}!</span>
      ...
      `;else if(o){const c=i&&Array.isArray(s.userAnswer)?s.userAnswer:[];l=`
        <div class="options">
          ${(i?t.options:I(t.options)).map((f,w)=>{const p=c.includes(f);return`
          <label class="option ${i?"disabled":""}">
            <input type="checkbox" name="answer" value="${f}" ${p?"checked":""} ${i?"disabled":""}>
    ...
    `,this.resultsDetails.innerHTML=`
      ...
      <div class="question-results">
        ${this.quizResults.questions.map((s,i)=>{const l=this.questions[i],c=this.getQuestionType(l),m=c==="multiple",h=c==="single",f=c==="number",w=c==="text";let p="Not answered",b=s.correctAnswer;m?(Array.isArray(s.userAnswer)&&s.userAnswer.length>0&&(p=s.userAnswer.join(", ")),b=Array.isArray(s.correctAnswer)?s.correctAnswer.join(", "):""):(h||f||w)&&(s.userAnswer!==null&&s.userAnswer!==void 0&&(p=s.userAnswer),b=s.correctAnswer),s.timeSpent&&Math.round(s.timeSpent/1e3);const q=s.timeSpent?r(s.timeSpent):"Not answered",z=s.status==="incorrect"?`<strong>${b}</strong>`:b;return`
