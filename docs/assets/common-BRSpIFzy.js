import"./style-B_s8H0E-.js";const $="your-github-client-id",q=window.location.origin+"/auth-callback.html",C="repo",T={questionCount:[{value:"2",label:"2 Questions"},{value:"5",label:"5 Questions",selected:!0},{value:"10",label:"10 Questions"},{value:"15",label:"15 Questions"},{value:"20",label:"20 Questions"}],timeLimit:[{value:"0",label:"No Limit",selected:!0},{value:"1",label:"1 Minute"},{value:"2",label:"2 Minutes"},{value:"3",label:"3 Minutes"},{value:"4",label:"4 Minutes"},{value:"5",label:"5 Minutes"},{value:"10",label:"10 Minutes"},{value:"15",label:"15 Minutes"},{value:"20",label:"20 Minutes"}]};function x(){return`
  ...
  `}let p=null,y=null;class I{constructor(t,e,i){this.duration=t,this.onTick=e,this.onComplete=i,this.timeLeft=t,this.interval=null,this.isRunning=!1}start(){this.isRunning=!0,this.interval=setInterval(()=>{this.timeLeft--,this.onTick&&this.onTick(this.timeLeft),this.timeLeft<=0&&(this.stop(),this.onComplete&&this.onComplete())},1e3)}stop(){this.isRunning=!1,this.interval&&(clearInterval(this.interval),this.interval=null)}getTimeLeft(){return this.timeLeft}formatTime(t){const e=Math.floor(t/60),i=t%60;return`${e.toString().padStart(2,"0")}:${i.toString().padStart(2,"0")}`}}class v{static async login(){const t=`https://github.com/login/oauth/authorize?client_id=${$}&redirect_uri=${encodeURIComponent(q)}&scope=${C}`;window.location.href=t}static async logout(){p=null,y=null,localStorage.removeItem("github_token"),localStorage.removeItem("github_user"),this.updateUI()}static async handleCallback(t){try{const e=await fetch("/api/github/auth",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({code:t})});if(e.ok){const i=await e.json();y=i.access_token,p=i.user,localStorage.setItem("github_token",y),localStorage.setItem("github_user",JSON.stringify(p)),this.updateUI()}}catch(e){console.error("Auth error:",e)}}static async loadUser(){const t=localStorage.getItem("github_token"),e=localStorage.getItem("github_user");t&&e&&(y=t,p=JSON.parse(e),this.updateUI())}static updateUI(){const t=document.getElementById("auth-section");t&&(p?t.innerHTML=`
      ...
      `;if(c||n?this.questionContainer.innerHTML=`
        <div class="question">
      ...
      `,s)this.showAnswerFeedback(t,a);else if(document.getElementById("submit-answer").addEventListener("click",()=>this.submitAnswer()),o){const r=document.getElementById("answer-input");r.addEventListener("keypress",d=>{d.key==="Enter"&&this.submitAnswer()}),r.focus()}this.prevBtn.disabled=this.currentQuestionIndex===0,this.nextBtn.style.display=this.currentQuestionIndex===this.questions.length-1?"none":"inline-block",this.finishBtn.style.display=this.currentQuestionIndex===this.questions.length-1?"inline-block":"none"}submitAnswer(){const t=this.questions[this.currentQuestionIndex],e=this.getQuestionType(t),i=e==="multiple",n=e==="single",c=document.getElementById("answer-feedback");let o;if(n){const s=document.querySelector('input[name="answer"]:checked');if(!s){c.innerHTML='<span class="error">Please select an answer</span>';return}o=s.value}else if(i){const s=Array.from(document.querySelectorAll('input[name="answer"]:checked'));if(s.length===0){c.innerHTML='<span class="error">Please select at least one answer</span>';return}o=s.map(l=>l.value)}else{const s=document.getElementById("answer-input");if(o=parseInt(s.value),isNaN(o)){c.innerHTML='<span class="error">Please enter a valid number</span>';return}}const a=Date.now()-this.questionStartTime;if(this.quizResults.submitAnswer(this.currentQuestionIndex,o,a),this.showAnswerFeedback(t,this.quizResults.questions[this.currentQuestionIndex]),n||i?document.querySelectorAll('input[name="answer"]').forEach(s=>{s.disabled=!0}):document.getElementById("answer-input").disabled=!0,document.getElementById("submit-answer").disabled=!0,this.currentQuestionIndex<this.questions.length-1){let s;if(i){const r=Array.isArray(t.correctAnswer)?t.correctAnswer:[];s=Array.isArray(o)&&o.length===r.length&&o.every(d=>r.includes(d))&&r.every(d=>o.includes(d))}else s=o===t.correctAnswer;const l=s?1e3:2e3,u=document.getElementById("next-question");if(u){const r=document.createElement("div");r.className="next-btn-progress",u.appendChild(r);const d=Date.now(),b=setInterval(()=>{const g=Date.now()-d,m=Math.min(g/l*100,100);r.style.width=`${m}%`,m>=100&&(clearInterval(b),this.nextQuestion())},50)}}}showAnswerFeedback(t,e){const i=this.getQuestionType(t),n=i==="multiple",c=i==="single",o=document.getElementById("answer-feedback");let a;if(n){const l=Array.isArray(t.correctAnswer)?t.correctAnswer:[],u=Array.isArray(e.userAnswer)?e.userAnswer:[];a=u.length===l.length&&u.every(r=>l.includes(r))&&l.every(r=>u.includes(r))}else a=e.userAnswer===t.correctAnswer;let s=`
      <span class="${a?"correct":"incorrect"}">
        ${a?"✓ Correct!":`✗ Incorrect. The correct answer is: ${n?t.correctAnswer.join(", "):t.correctAnswer}`}
      </span>
    `;(c||n)&&t.explanation&&(s+=`<div class="explanation">${t.explanation}</div>`),o.innerHTML=s}previousQuestion(){this.currentQuestionIndex>0&&(this.currentQuestionIndex--,this.displayQuestion())}nextQuestion(){this.currentQuestionIndex<this.questions.length-1&&(this.currentQuestionIndex++,this.displayQuestion())}finishQuiz(){this.quizResults.endQuiz(),this.timer&&this.timer.stop(),this.stopElapsedTimeTracking(),this.showResults()}timeUp(){alert("Time is up! Quiz will be submitted automatically."),this.finishQuiz()}startElapsedTimeTracking(){this.elapsedTimeInterval=setInterval(()=>{const t=Date.now()-this.quizStartTime;this.updateElapsedTime(t)},1e3)}stopElapsedTimeTracking(){this.elapsedTimeInterval&&(clearInterval(this.elapsedTimeInterval),this.elapsedTimeInterval=null),this.timerDisplay&&(this.timerDisplay.textContent="")}updateElapsedTime(t){const e=Math.floor(t/1e3),i=Math.floor(e/60),n=e%60;this.timerDisplay.textContent=`Time: ${i}:${n.toString().padStart(2,"0")}`}updateTimer(t){this.timerDisplay.textContent=this.timer.formatTime(t),t<=60&&this.timerDisplay.classList.add("warning")}showResults(){this.configSection.style.display="none",this.quizSection.style.display="none",this.resultsSection.style.display="block";const t=this.quizResults.getResults(),e=this.quizResults.questions.filter(s=>s.timeSpent),i=e.reduce((s,l)=>s+l.timeSpent,0),n=e.length>0?i/e.length:0,c=e.length>0?Math.min(...e.map(s=>s.timeSpent)):0,o=e.length>0?Math.max(...e.map(s=>s.timeSpent)):0,a=s=>{const l=Math.round(s/1e3),u=Math.floor(l/60),r=l%60;return u>0?`${u}m ${r}s`:`${l}s`};this.resultsSummary.innerHTML=`
      <div class="results-stats">
        ...
        <div class="stat">
          ...
          <span class="stat-value">${a(n)}</span>
        </div>
        <div class="stat">
    ...
    `,this.resultsDetails.innerHTML=`
      ...
      <div class="question-results">
        ${this.quizResults.questions.map((s,l)=>{const u=this.questions[l],r=this.getQuestionType(u),d=r==="multiple",b=r==="single",g=r==="number";let m="Not answered",w=s.correctAnswer;d?(Array.isArray(s.userAnswer)&&s.userAnswer.length>0&&(m=s.userAnswer.join(", ")),w=Array.isArray(s.correctAnswer)?s.correctAnswer.join(", "):""):(b||g)&&(s.userAnswer!==null&&s.userAnswer!==void 0&&(m=s.userAnswer),w=s.correctAnswer),s.timeSpent&&Math.round(s.timeSpent/1e3);const Q=s.timeSpent?a(s.timeSpent):"Not answered",S=s.status==="incorrect"?`<strong>${w}</strong>`:w;return`
