import"./style-DMDr5Rx3.js";import{i as p}from"./header-D8eS3KwV.js";import{q as r}from"./quiz-storage-DIh2LGXt.js";p();class v{constructor(){this.currentView="list",this.currentResultId=null,this.init()}init(){this.createViewerHTML(),this.bindEvents(),this.showResultsList()}createViewerHTML(){const t=document.getElementById("quiz-results-viewer");t&&t.remove();const s=`
    ...
    `}renderResultsList(){const t=document.getElementById("results-list-container");if(!t)return;const s=r.getAllResults();if(s.length===0){t.innerHTML=`
        <div class="no-results">
          <p>No quiz results found.</p>
          <p>Complete some quizzes to see your results here!</p>
        </div>
      `;return}const e=s.map(i=>{const a=Math.round(i.correctAnswers/i.totalQuestions*100),l=a>=80?"#6bcf7f":a>=60?"#ffd93d":"#ff6b6b";return`
        <div class="result-item" data-result-id="${i.id}">
          ...
          <div class="result-summary">
            <div class="result-stats">
              ...
              <span class="stat score" style="color: ${l}">
    ...
    `}renderResultDetail(t){const s=document.getElementById("result-detail-container");if(!s)return;const e=r.getResultById(t);if(!e){s.innerHTML="<p>Result not found.</p>";return}const i=Math.round(e.correctAnswers/e.totalQuestions*100),a=i>=80?"#6bcf7f":i>=60?"#ffd93d":"#ff6b6b",l=e.questions.map((n,u)=>{const o=n.isCorrect,d=o?"#6bcf7f":"#ff6b6b",m=o?"✓ Correct":"✗ Incorrect";return`
    ...
    `}deleteResult(t){confirm("Are you sure you want to delete this quiz result?")&&(r.deleteResult(t)?(this.currentView==="detail"&&this.currentResultId===t?this.showResultsList():this.renderResultsList(),this.updateStorageStats()):alert("Failed to delete quiz result."))}clearAllResults(){confirm("Are you sure you want to delete ALL quiz results? This action cannot be undone.")&&(r.clearAllResults()?this.showResultsList():alert("Failed to clear quiz results."))}formatBytes(t){if(t===0)return"0 Bytes";const s=1024,e=["Bytes","KB","MB","GB"],i=Math.floor(Math.log(t)/Math.log(s));return parseFloat((t/Math.pow(s,i)).toFixed(2))+" "+e[i]}formatTime(t){if(!t)return"0s";const s=Math.floor(t/1e3),e=Math.floor(s/60),i=s%60;return e>0?`${e}m ${i}s`:`${s}s`}}const c=new v;window.quizResultsViewer=c;c.showResultsList();
